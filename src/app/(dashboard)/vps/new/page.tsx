...
const OS_LIST = [
  { id:"ubuntu-22-04", label:"Ubuntu 22.04 LTS", icon:"Linux" },
  { id:"ubuntu-20-04", label:"Ubuntu 20.04 LTS", icon:"Linux" },
  { id:"debian-12", label:"Debian 12 (Bookworm)", icon:"Debian" },
  { id:"centos-stream-9", label:"CentOS Stream 9", icon:"CentOS" },
  { id:"rocky-linux-9", label:"Rocky Linux 9", icon:"Rocky" },
  { id:"windows-2022", label:"Windows Server 2022", icon:"Windows" },
];
...
export default function BuyVpsPage() {
  ...
  useEffect(() => {
    Promise.all([
      ...
      fetch("/api/vps/providers").then(r=>r.json()),
    ]).then(([p, pv]) => {
      setPackages(p.data||[]); setProviders(pv.data||[]); setLoading(false);
    });
  }, []);

  const filteredPkgs = selectedProvider === "all" ? packages : packages.filter(p => p.providerId === selectedProvider);
  ...
  async function checkCoupon() {
    ...
    setCouponResult(data); setCouponLoading(false);
  }
            ...
            <div style={{ display:"flex", flexDirection:"column", gap:8 }}>
              {filteredPkgs.map(p => (
                <div key={p.id} onClick={()=>setSelectedPkg(p.id)} style={{
                  ...
                  background:selectedPkg===p.id?"var(--accent-soft)":"var(--bg-elevated)",
                  ...
                  <div style={{ flex:1 }}>
                    ...
                    <div style={{ fontSize:12, color:"var(--text-muted)", display:"flex", gap:12 }}>
                      <span>{p.cpu} vCPU</span><span>{p.ram} GB RAM</span><span>{p.storage} GB SSD</span>
                      <span>{p.bandwidth === 0 ? "∞ BW" : p.bandwidth + " GB BW"}</span>
                    </div>
                  </div>
            ...
            <div style={{ display:"grid", gridTemplateColumns:"repeat(3,1fr)", gap:8 }}>
              {OS_LIST.map(os => (
                <button key={os.id} onClick={()=>setSelectedOS(os.id)} style={{
                  padding:"10px 12px", borderRadius:"var(--radius-md)", border:`1.5px solid ${selectedOS===os.id?"var(--accent)":"var(--border)"}`,
                  background:selectedOS===os.id?"var(--accent-soft)":"var(--bg-elevated)",
                  color:selectedOS===os.id?"var(--accent)":"var(--text-secondary)", cursor:"pointer", transition:"all 0.12s",
        ...
        <div style={{ position:"sticky", top:24 }}>
          <div style={{ background:"var(--bg-surface)", border:"1px solid var(--border)", borderRadius:"var(--radius-lg)", padding:"20px" }}>
            ...
            {pkg ? (
              <>
                <div style={{ background:"var(--bg-elevated)", borderRadius:"var(--radius-md)", padding:"12px 14px", marginBottom:14 }}>
                  ...
                  <div style={{ fontSize:12, color:"var(--text-muted)" }}>{pkg.cpu} vCPU · {pkg.ram}GB RAM · {pkg.storage}GB SSD</div>
                  <div style={{ fontSize:12, color:"var(--text-muted)"}}>{OS_LIST.find(o=>o.id===selectedOS)?.label}</div>
                </div>
            ...
            <button onClick={order} disabled={ordering||!selectedPkg||!hostname.trim()} style={{
              ...
              background: (ordering||!selectedPkg||!hostname.trim()) ? "rgba(79,124,255,0.3)" : "var(--accent)",
