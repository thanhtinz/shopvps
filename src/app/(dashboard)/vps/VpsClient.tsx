  ...
  return (
    <div style={{ maxWidth: 1100, margin: "0 auto" }}>
      ...
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 24 }}>
        <div>
          <h1 style={{ fontSize: 20, fontWeight: 800, color: "var(--text-primary)", letterSpacing: "-0.03em", marginBottom: 2 }}>{t("Máy chủ VPS")}</h1>
          <p style={{ color: "var(--text-muted)", fontSize: 13 }}>{orders.length} {t("VPS trong tài khoản")}</p>
        </div>
        ...
        >
          <Icon d="M12 5v14 M5 12h14" /> {t("Mua VPS mới")}
        </button>
      </div>
      ...
      {orders.length === 0 && (
        <div style={{ textAlign: "center", padding: "80px 20px", background: "var(--bg-surface)", border: "1px solid var(--border)", borderRadius: "var(--radius-xl)" }}>
          ...
          <h3 style={{ fontSize: 16, fontWeight: 700, color: "var(--text-primary)", marginBottom: 8 }}>{t("Chưa có VPS nào")}</h3>
          <p style={{ color: "var(--text-muted)", fontSize: 13, marginBottom: 24 }}>{t("Mua VPS đầu tiên để bắt đầu")}</p>
          ...
          >
            <div style={{ display: "flex", alignItems: "flex-start", gap: 16, flexWrap: "wrap" }}>
              ...
              <div style={{ display: "flex", alignItems: "center", gap: 14, flex: "1 1 280px" }}>
                ...
                <div>
                  ...
                  <div style={{ fontSize: 12, color: "var(--text-muted)", fontFamily: "var(--font-mono)" }}>
                    {vps.ipAddress || t("Đang cấp phát IP...")}
                  </div>
                </div>
              </div>
              ...
              <div style={{ flex: "0 0 auto", textAlign: "right" }}>
                <div style={{ fontSize: 15, fontWeight: 800, color: "var(--text-primary)", marginBottom: 2 }}>{formatCurrency(vps.price)}/{t("tháng")}</div>
                <div style={{ fontSize: 11.5, color: "var(--text-muted)", marginBottom: 8 }}>
                  {t("HH:")} {vps.expiresAt ? formatDate(vps.expiresAt) : "—"}
                </div>
                <Link href={`/vps/${vps.id}`} style={{ fontSize: 12.5, fontWeight: 600, color: "var(--accent)", textDecoration: "none" }}>{t("Quản lý")} →</Link>
              </div>
            </div>
            ...
            {vps.status === "ACTIVE" && (
              <div style={{ display: "flex", gap: 8, marginTop: 16, paddingTop: 16, borderTop: "1px solid var(--border)", flexWrap: "wrap" }}>
                {[
                  { action: "power_off", label: t("Tắt"), icon: "M18.36 6.64a9 9 0 11-12.73 0 M12 2v10", color: "var(--red)" },
                  { action: "restart", label: t("Khởi động lại"), icon: "M23 4v6h-6 M1 20v-6h6 M3.51 9a9 9 0 0114.85-3.36L23 10 M1 14l4.64 4.36A9 9 0 0020.49 15", color: "var(--yellow)" },
                  { action: "rebuild", label: t("Cài lại OS"), icon: "M21 2H3v16h5v4l4-4h5l4-4V2zm-10 9V7m4 4V7", color: "var(--purple)" },
                ].map(btn => (
        ...
        <div style={{
          ...
          <div style={{
            ...
            <p style={{ fontSize: 13, color: "var(--text-secondary)", marginBottom: 20 }}>
              {t("Bạn chắc chắn muốn thực hiện")} <strong style={{ color: "var(--text-primary)" }}>{showConfirm.action.replace("_", " ")}</strong>?
            </p>
