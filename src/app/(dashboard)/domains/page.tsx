...
export default function DomainsPage() {
  ...
  async function register(type: "register"|"transfer") {
    ...
    setResult(null); setQuery(""); setMsg("Đã tạo đơn tên miền, đang chờ xử lý."); load();
  }
  ...
  return (
    <div style={{ maxWidth:1000, margin:"0 auto" }}>
      <h1 style={{ fontSize:20, fontWeight:800, color:"var(--text-primary)", letterSpacing:"-0.03em", marginBottom:4 }}>Tên miền</h1>
      <p style={{ color:"var(--text-muted)", fontSize:13, marginBottom:20 }}>Đăng ký, chuyển về và quản lý tên miền</p>
      ...
      <div style={{ background:"var(--bg-surface)", border:"1px solid var(--border)", borderRadius:"var(--radius-lg)", padding:"20px", marginBottom:24 }}>
        <form onSubmit={check} style={{ display:"flex", gap:10 }}>
          <input value={query} onChange={e=>setQuery(e.target.value)} placeholder="vd: tencuaban.com" style={inp as any}/>
          <button type="submit" disabled={checking} style={{ padding:"10px 20px", background:"var(--accent)", border:"none", borderRadius:"var(--radius-md)", color:"white", fontWeight:600, fontSize:13.5, cursor:"pointer", whiteSpace:"nowrap" }}>{checking?"Đang kiểm tra...":"Kiểm tra"}</button>
        </form>
            ...
            <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", flexWrap:"wrap", gap:12 }}>
              <div>
                ...
                <div style={{ fontSize:12.5, marginTop:3 }}>
                  {result.available === true && <span style={{ color:"var(--green)" }}>● Còn trống — {formatCurrency(result.registerPrice)}/năm</span>}
                  {result.available === false && <span style={{ color:"var(--red)" }}>● Đã được đăng ký {result.transferPrice ? `— có thể transfer ${formatCurrency(result.transferPrice)}` : ""}</span>}
                  {result.unknown && <span style={{ color:"var(--yellow)" }}>● Không kiểm tra được tự động — bạn vẫn có thể đặt đơn</span>}
                </div>
              </div>
              <div style={{ display:"flex", alignItems:"center", gap:8 }}>
                <select value={years} onChange={e=>setYears(parseInt(e.target.value))} style={{ ...inp as any, width:90 }}>
                  {[1,2,3,5,10].map(y=><option key={y} value={y}>{y} năm</option>)}
                </select>
                {result.available !== false ? (
                  <button onClick={()=>register("register")} disabled={busy} style={{ padding:"10px 18px", background:"var(--accent)", border:"none", borderRadius:"var(--radius-md)", color:"white", fontWeight:600, fontSize:13, cursor:"pointer" }}>{busy?"...":"Đăng ký"}</button>
      ...
      <div style={{ background:"var(--bg-surface)", border:"1px solid var(--border)", borderRadius:"var(--radius-lg)", overflow:"hidden" }}>
        <div style={{ padding:"14px 18px", borderBottom:"1px solid var(--border)", fontSize:14, fontWeight:700, color:"var(--text-primary)" }}>Tên miền của tôi ({domains.length})</div>
        {loading ? <div className="skeleton" style={{ height:160, margin:16, borderRadius:"var(--radius-md)" }}/> : domains.length===0 ? (
          <div style={{ textAlign:"center", padding:"50px 20px", color:"var(--text-muted)", fontSize:13 }}>Bạn chưa có tên miền nào</div>
        ) : (
          <table style={{ width:"100%", borderCollapse:"collapse" }}>
            <thead><tr style={{ borderBottom:"1px solid var(--border)" }}>
              {["Tên miền","Trạng thái","Hết hạn","Tự gia hạn",""].map(h=>(
                <th key={h} style={{ padding:"11px 16px", textAlign:"left", fontSize:11, fontWeight:700, color:"var(--text-muted)", textTransform:"uppercase", letterSpacing:"0.05em" }}>{h}</th>
              ))}
            </tr></thead>
            <tbody>
              {domains.map(d=>(
                <tr key={d.id} style={{ borderBottom:"1px solid var(--border)" }}>
                  ...
                  <td style={{ padding:"12px 16px" }}><Badge color={statusColor[d.status]||"gray"}>{statusLabel[d.status]||d.status}</Badge></td>
                  <td style={{ padding:"12px 16px", fontSize:12.5, color:"var(--text-muted)" }}>{d.expiresAt?formatDate(d.expiresAt):"—"}</td>
                  <td style={{ padding:"12px 16px", fontSize:12.5, color:"var(--text-secondary)" }}>{d.autoRenew?"Bật":"Tắt"}</td>
                  <td style={{ padding:"12px 16px", textAlign:"right" }}>
                    <button onClick={()=>openManage(d)} style={{ padding:"6px 12px", borderRadius:"var(--radius-sm)", border:"1px solid var(--border)", background:"var(--bg-elevated)", color:"var(--text-secondary)", fontSize:12.5, cursor:"pointer" }}>Quản lý</button>
                  </td>
                </tr>
              ))}
            </tbody>
      ...
      {manage && (
        <div style={{ position:"fixed", inset:0, background:"rgba(0,0,0,0.6)", backdropFilter:"blur(4px)", display:"flex", alignItems:"center", justifyContent:"center", zIndex:50 }} onClick={()=>setManage(null)}>
          <div style={{ background:"var(--bg-elevated)", border:"1px solid var(--border)", borderRadius:"var(--radius-xl)", padding:"26px", width:480, maxWidth:"92vw" }} onClick={e=>e.stopPropagation()}>
            ...
            <div style={{ fontSize:12.5, color:"var(--text-muted)", marginBottom:18 }}>{statusLabel[manage.status]} · Hết hạn {manage.expiresAt?formatDate(manage.expiresAt):"—"}</div>

            <label style={{ display:"block", fontSize:11, fontWeight:700, color:"var(--text-muted)", letterSpacing:"0.06em", textTransform:"uppercase", marginBottom:6 }}>Nameservers (mỗi dòng/phẩy)</label>
            ...
            <label style={{ display:"flex", alignItems:"center", gap:8, fontSize:13, color:"var(--text-secondary)", cursor:"pointer", marginBottom:18 }}>
              <input type="checkbox" checked={manage.autoRenew} onChange={e=>setManage({...manage, autoRenew:e.target.checked})}/> Tự động gia hạn
            </label>

            <div style={{ display:"flex", gap:10, marginBottom:10 }}>
              <button onClick={()=>saveManage({ nameservers: ns, autoRenew: manage.autoRenew })} style={{ flex:1, padding:"10px", background:"var(--accent)", border:"none", borderRadius:"var(--radius-md)", color:"white", fontWeight:600, fontSize:13, cursor:"pointer" }}>Lưu</button>
              {manage.status==="ACTIVE" && <button onClick={()=>saveManage({ action:"renew", years:1 })} style={{ flex:1, padding:"10px", background:"var(--bg-surface)", border:"1px solid var(--border)", borderRadius:"var(--radius-md)", color:"var(--text-primary)", fontWeight:600, fontSize:13, cursor:"pointer" }}>Gia hạn 1 năm</button>}
            </div>
            <button onClick={()=>setManage(null)} style={{ width:"100%", padding:"9px", background:"transparent", border:"none", color:"var(--text-muted)", fontSize:12.5, cursor:"pointer" }}>Đóng</button>
          </div>
        </div>
      )}
    </div>
