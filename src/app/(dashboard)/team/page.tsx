...
export default function TeamPage() {
  ...
  async function invite(e: React.FormEvent) {
    ...
    if (data.success) {
      setSuccess(`Đã mời ${inviteEmail} thành công!`);
  ...
  return (
    <div style={{ maxWidth:800, margin:"0 auto" }}>
      <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:24 }}>
        <div>
          <h1 style={{ fontSize:20, fontWeight:800, color:"var(--text-primary)", letterSpacing:"-0.03em", marginBottom:2 }}>Quản lý Team</h1>
          <p style={{ color:"var(--text-muted)", fontSize:13 }}>Chia sẻ quyền truy cập VPS/Hosting với thành viên</p>
        </div>
        <button onClick={()=>setShowInvite(true)} style={{ display:"flex", alignItems:"center", gap:7, padding:"9px 16px", background:"var(--accent)", border:"none", borderRadius:"var(--radius-md)", color:"white", fontSize:13, fontWeight:600, cursor:"pointer" }}>
          <Icon d="M18 21a8 8 0 00-16 0 M10 8a4 4 0 108 0 4 4 0 00-8 0 M22 21v-2a4 4 0 00-3-3.87 M16 3.13a4 4 0 010 7.75"/> Mời thành viên
        </button>
      </div>
        ...
        <div style={{ background:"var(--bg-surface)", border:"1px solid var(--border)", borderRadius:"var(--radius-lg)", padding:"20px", marginBottom:18 }}>
          <div style={{ display:"flex", alignItems:"center", gap:12, marginBottom:16 }}>
            ...
            <div>
              ...
              <div style={{ fontSize:12, color:"var(--text-muted)" }}>{team.members?.length || 0} thành viên</div>
            </div>
          </div>

          {team.members?.length === 0 ? (
            <div style={{ textAlign:"center", padding:"32px 0", color:"var(--text-muted)", fontSize:13 }}>
              Chưa có thành viên nào. Mời người dùng để bắt đầu cộng tác.
            </div>
          ) : (
              ...
              {team.members?.map((m: any) => (
                <div key={m.id} style={{ display:"flex", alignItems:"center", gap:12, padding:"12px 14px", background:"var(--bg-elevated)", borderRadius:"var(--radius-md)", border:"1px solid var(--border)" }}>
                  ...
                  <Badge color={m.status==="ACCEPTED"?"green":m.status==="PENDING"?"yellow":"red"}>
                    {m.status==="ACCEPTED"?"Đã tham gia":m.status==="PENDING"?"Chờ xác nhận":"Từ chối"}
                  </Badge>
                  <div style={{ display:"flex", flexWrap:"wrap", gap:4, maxWidth:240 }}>
                    {PERMS.filter(p => m[p.key]).map(p => (
                      <span key={p.key} style={{ padding:"1px 6px", borderRadius:99, fontSize:10.5, background:"var(--accent-soft)", color:"var(--accent)", fontWeight:600 }}>{p.label}</span>
                    ))}
                  </div>
                </div>
              ))}
      ...
      {!team && (
        <div style={{ textAlign:"center", padding:"60px 20px", background:"var(--bg-surface)", border:"1px solid var(--border)", borderRadius:"var(--radius-xl)" }}>
          ...
          <h3 style={{ fontSize:16, fontWeight:700, color:"var(--text-primary)", marginBottom:8 }}>Chưa có team</h3>
          <p style={{ color:"var(--text-muted)", fontSize:13, marginBottom:24 }}>Mời thành viên đầu tiên để tạo team</p>
          <button onClick={()=>setShowInvite(true)} style={{ padding:"10px 24px", background:"var(--accent)", border:"none", borderRadius:"var(--radius-md)", color:"white", fontWeight:600, cursor:"pointer" }}>Mời ngay</button>
        </div>
      )}
      ...
      {showInvite && (
        <div style={{ position:"fixed", inset:0, background:"rgba(0,0,0,0.6)", backdropFilter:"blur(4px)", display:"flex", alignItems:"center", justifyContent:"center", zIndex:50 }} onClick={()=>setShowInvite(false)}>
          <div style={{ background:"var(--bg-elevated)", border:"1px solid var(--border)", borderRadius:"var(--radius-xl)", padding:"28px", width:500, maxWidth:"90vw" }} onClick={e=>e.stopPropagation()}>
            <h3 style={{ fontSize:16, fontWeight:700, color:"var(--text-primary)", marginBottom:20 }}>Mời thành viên</h3>
            <form onSubmit={invite}>
              <div style={{ marginBottom:16 }}>
                <label style={{ display:"block", fontSize:11, fontWeight:700, color:"var(--text-muted)", letterSpacing:"0.08em", textTransform:"uppercase", marginBottom:6 }}>Email thành viên</label>
              ...
              <div style={{ marginBottom:20 }}>
                <label style={{ display:"block", fontSize:11, fontWeight:700, color:"var(--text-muted)", letterSpacing:"0.08em", textTransform:"uppercase", marginBottom:10 }}>Phân quyền</label>
              ...
              <div style={{ display:"flex", gap:10 }}>
                <button type="button" onClick={()=>setShowInvite(false)} style={{ flex:1, padding:"10px", background:"var(--bg-hover)", border:"1px solid var(--border)", borderRadius:"var(--radius-md)", color:"var(--text-secondary)", cursor:"pointer", fontSize:13 }}>Huỷ</button>
