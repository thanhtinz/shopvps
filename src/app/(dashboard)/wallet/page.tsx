...
export default function WalletPage() {
  ...
  return (
    <div style={{ maxWidth: 900, margin: "0 auto" }}>
      <h1 style={{ fontSize: 20, fontWeight: 800, letterSpacing: "-0.03em", marginBottom: 20 }}>Ví & Thanh toán</h1>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 14, marginBottom: 24 }}>
        <div style={{ background: "linear-gradient(135deg,rgba(79,124,255,0.15),rgba(124,58,237,0.08))", border: "1px solid rgba(79,124,255,0.25)", borderRadius: "var(--radius-lg)", padding: "24px" }}>
          <div style={{ fontSize: 12, color: "var(--text-muted)", marginBottom: 8 }}>Số dư ví chính</div>
        ...
        <div style={{ background: "var(--bg-surface)", border: "1px solid var(--border)", borderRadius: "var(--radius-lg)", padding: "24px" }}>
          <div style={{ fontSize: 12, color: "var(--text-muted)", marginBottom: 8 }}>Ví hoa hồng</div>
       ...
       <div>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 18 }}>
          ...
          <div style={{ background: "var(--bg-surface)", border: "1px solid var(--border)", borderRadius: "var(--radius-lg)", padding: "20px" }}>
            <h3 style={{ fontSize: 13.5, fontWeight: 700, marginBottom: 16 }}>Chọn số tiền nạp</h3>
            ...
            {depositInfo?.bonuses?.length > 0 && (
              <div style={{ marginTop: 14, padding: "12px", background: "var(--green-soft)", borderRadius: "var(--radius-md)", border: "1px solid rgba(34,197,94,0.2)" }}>
                <div style={{ fontSize: 12, fontWeight: 600, color: "var(--green)", marginBottom: 6 }}> Khuyến mãi nạp tiền</div>
                {depositInfo.bonuses.map((b: any, i: number) => (
                  <div key={i} style={{ fontSize: 11.5, color: "var(--text-secondary)", marginBottom: 2 }}>
                    Nạp từ {formatCurrency(b.minAmount)} → Bonus <strong style={{ color: "var(--green)" }}>+{b.bonusPercent}%</strong>
                  </div>
                ))}
              </div>
            )}
            ...
            {bonus && finalAmount > 0 && (
              <div style={{ marginTop: 10, padding: "10px 12px", background: "var(--accent-soft)", borderRadius: "var(--radius-md)", fontSize: 12.5 }}>
                ★ Bạn nhận thêm <strong style={{ color: "var(--accent)" }}>{formatCurrency(Math.floor(finalAmount * Number(bonus.bonusPercent) / 100))}</strong> bonus!
              </div>
            )}
          </div>
            ...
            <table style={{ width: "100%", borderCollapse: "collapse" }}>
              <thead>
                <tr style={{ borderBottom: "1px solid var(--border)" }}>
                  {["Thời gian", "Loại", "Mô tả", "Số tiền", "Số dư sau"].map(h => (
              ...
              <tbody>
                {transactions.length === 0 ? (
                  <tr><td colSpan={5} style={{ textAlign: "center", padding: "40px", color: "var(--text-muted)", fontSize: 13 }}>Chưa có giao dịch</td></tr>
                ) : transactions.map(tx => (
