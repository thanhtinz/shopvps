...
import { isKnownType, groupOfType, typeAutoActivates } from "@/lib/products";
...
export async function GET() {
  ...
  const products = await prisma.product.findMany({ orderBy: [{ group: "asc" }, { category: "asc" }, { sortOrder: "asc" }] });
...
export async function POST(req: NextRequest) {
  ...
  if (!isKnownType(category) || !b.name || b.priceMonthly == null) return NextResponse.json({ error: t("Thiếu thông tin") }, { status: 400 });
  ...
  const product = await prisma.product.create({
    data: {
      group: groupOfType(category), category,
      name: String(b.name), slug: String(b.slug || b.name).toLowerCase().replace(/[^a-z0-9]+/g, "-"),
      ...
      autoActivate: typeof b.autoActivate === "boolean" ? b.autoActivate : typeAutoActivates(category),
