...
export async function POST(req: NextRequest) {
  ...
  if (!Number.isFinite(amt) || amt <= 0) return NextResponse.json({ error: "Số tiền không hợp lệ" }, { status: 400 });

  const currency = await getCurrency(currencyCode || "VND");
  if (!currency) return NextResponse.json({ error: "Tiền tệ không hợp lệ" }, { status: 400 });

  const gw = getGateway(gatewayCode);
  const row = await prisma.paymentGateway.findUnique({ where: { code: gatewayCode || "" } });
  if (!gw || !row?.isActive) return NextResponse.json({ error: "Cổng thanh toán không khả dụng" }, { status: 400 });

  const amountBase = toBase(amt, currency);
  if (amountBase < 1000) return NextResponse.json({ error: "Số tiền tối thiểu 1.000đ" }, { status: 400 });
  ...
  try {
    ...
    return NextResponse.json({ error: e?.message || "Không khởi tạo được thanh toán" }, { status: 500 });
  }
}
