...
export async function POST(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  if (!vps.providerVpsId) return NextResponse.json({ error: t("VPS chưa được khởi tạo") }, { status: 400 });
  ...
  try {
    ...
    switch (action as Action) {
      ...
      case "restart":
        ...
        break;
      case "rebuild":
        if (!osId) return NextResponse.json({ error: t("Thiếu OS") }, { status: 400 });
      ...
      case "change_password":
        if (!password) return NextResponse.json({ error: t("Thiếu mật khẩu") }, { status: 400 });
