...
function RegisterForm() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    if (password !== confirmPw) { setError(t("Mật khẩu xác nhận không khớp")); return; }
    ...
    try {
      ...
      setSuccess(true);
    } catch { setError(t("Không thể kết nối máy chủ")); }
  ...
  if (success) return (
    <div style={{ textAlign:"center" }}>
      <div style={{ background:"rgba(13,17,23,0.95)", border:"1px solid rgba(255,255,255,0.07)", borderRadius:20, padding:"40px 28px" }}>
        ...
        <h2 style={{ fontSize:18, fontWeight:800, color:"#e8edf5", marginBottom:8 }}>{t("Kiểm tra email")}</h2>
        <p style={{ color:"#8896aa", fontSize:13, marginBottom:20, lineHeight:1.6 }}>{t("Chúng tôi đã gửi link xác thực đến")} <strong style={{ color:"#e8edf5" }}>{email}</strong>. {t("Vui lòng kiểm tra và xác thực tài khoản.")}</p>
        <Link href="/login" style={{ display:"inline-block", padding:"10px 24px", background:"#4f7cff", borderRadius:10, color:"white", textDecoration:"none", fontSize:13, fontWeight:600 }}>{t("Đăng nhập →")}</Link>
      </div>
    </div>
  );
  ...
  return (
    <div>
      <div style={{ textAlign:"center", marginBottom:28 }}>
        ...
        <h1 style={{ fontSize:22, fontWeight:800, color:"#e8edf5", letterSpacing:"-0.03em", marginBottom:4 }}>{t("Tạo tài khoản")}</h1>
        <p style={{ color:"#4a5568", fontSize:13 }}>{t("Bắt đầu sử dụng ShopVPS ngay hôm nay")}</p>
      </div>
      ...
      <div style={{ background:"rgba(13,17,23,0.95)", border:"1px solid rgba(255,255,255,0.07)", borderRadius:20, padding:"28px", boxShadow:"0 20px 60px rgba(0,0,0,0.4)" }}>
        {ref && (
          <div style={{ background:"rgba(79,124,255,0.08)", border:"1px solid rgba(79,124,255,0.2)", borderRadius:8, padding:"9px 12px", marginBottom:16, fontSize:12.5, color:"#818cf8" }}>
             {t("Bạn được giới thiệu — sẽ nhận ưu đãi đặc biệt")}
          </div>
        )}
        ...
        <form onSubmit={handleSubmit}>
          {[
            { label:t("Họ tên"), val:name, set:setName, type:"text", ph:t("Nguyễn Văn A") },
          ...
          <div style={{ marginBottom:13 }}>
            <label style={{ display:"block", fontSize:11, fontWeight:700, color:"#4a5568", letterSpacing:"0.08em", textTransform:"uppercase", marginBottom:6 }}>{t("Mật khẩu")}</label>
            <div style={{ position:"relative" }}>
              <input type={showPw?"text":"password"} value={password} onChange={e=>setPassword(e.target.value)} placeholder={t("Tối thiểu 8 ký tự")} required style={{ ...inputStyle, paddingRight:42 }}
                onFocus={e=>e.target.style.borderColor="rgba(79,124,255,0.5)"} onBlur={e=>e.target.style.borderColor="rgba(255,255,255,0.08)"}/>
          ...
          <div style={{ marginBottom:20 }}>
            <label style={{ display:"block", fontSize:11, fontWeight:700, color:"#4a5568", letterSpacing:"0.08em", textTransform:"uppercase", marginBottom:6 }}>{t("Xác nhận mật khẩu")}</label>
          ...
          <p style={{ textAlign:"center", color:"#4a5568", fontSize:11.5, marginTop:12, lineHeight:1.5 }}>
            {t("Bằng cách đăng ký, bạn đồng ý với")} <Link href="/terms" style={{ color:"#4f7cff", textDecoration:"none" }}>{t("Điều khoản")}</Link> {t("và")} <Link href="/privacy" style={{ color:"#4f7cff", textDecoration:"none" }}>{t("Chính sách")}</Link>
          </p>
        </form>

        <p style={{ textAlign:"center", color:"#4a5568", fontSize:13, marginTop:18, borderTop:"1px solid rgba(255,255,255,0.07)", paddingTop:18 }}>
          {t("Đã có tài khoản?")}{" "}<Link href="/login" style={{ color:"#4f7cff", textDecoration:"none", fontWeight:600 }}>{t("Đăng nhập")}</Link>
        </p>
      </div>
    </div>
  );
