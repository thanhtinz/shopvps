...
export default function ForgotPasswordPage() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      else setError(data.error || t("Đã có lỗi xảy ra"));
  ...
  return (
    <div>
      <div style={{ textAlign:"center", marginBottom:28 }}>
        ...
        <h1 style={{ fontSize:22, fontWeight:800, color:"#e8edf5", letterSpacing:"-0.03em", marginBottom:4 }}>{t("Quên mật khẩu")}</h1>
        <p style={{ color:"#4a5568", fontSize:13 }}>{t("Nhập email để nhận link đặt lại")}</p>
      </div>
      <div style={{ background:"rgba(13,17,23,0.95)", border:"1px solid rgba(255,255,255,0.07)", borderRadius:20, padding:"28px" }}>
        {!sent ? (
          ...
          <div style={{ textAlign:"center", padding:"20px 0" }}>
            ...
            <h3 style={{ fontSize:16, fontWeight:700, color:"#e8edf5", marginBottom:8 }}>{t("Kiểm tra email")}</h3>
            <p style={{ color:"#8896aa", fontSize:13, lineHeight:1.6, marginBottom:20 }}>{t("Chúng tôi đã gửi link đến")} <strong style={{ color:"#e8edf5" }}>{email}</strong>. {t("Kiểm tra cả hộp thư spam.")}</p>
            <Link href="/login" style={{ color:"#4f7cff", textDecoration:"none", fontSize:13, fontWeight:600 }}>{t("← Quay lại đăng nhập")}</Link>
          </div>
        )}
      </div>
    </div>
