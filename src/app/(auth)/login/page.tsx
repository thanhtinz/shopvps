...
export default function LoginPage() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    if (res?.error) { setError("Email hoặc mật khẩu không đúng"); setLoading(false); return; }
  ...
  return (
    <div>
      <div style={{ textAlign: "center", marginBottom: 28 }}>
        ...
        <h1 style={{ fontSize:22, fontWeight:800, color:"#e8edf5", letterSpacing:"-0.03em", marginBottom:4 }}>{show2FA ? "Xác thực 2 lớp" : "Đăng nhập"}</h1>
        <p style={{ color:"#4a5568", fontSize:13 }}>{show2FA ? "Nhập mã từ ứng dụng xác thực" : "Chào mừng trở lại!"}</p>
      </div>
        ...
        {!show2FA ? <>
          ...
          <form onSubmit={handleSubmit}>
            ...
            <div style={{ marginBottom:20 }}>
              <div style={{ display:"flex", justifyContent:"space-between", marginBottom:6 }}>
                <label style={{ fontSize:11, fontWeight:700, color:"#4a5568", letterSpacing:"0.08em", textTransform:"uppercase" }}>Mật khẩu</label>
                <Link href="/forgot-password" style={{ fontSize:12, color:"#4f7cff", textDecoration:"none" }}>Quên mật khẩu?</Link>
              </div>
          ...
          <form onSubmit={handleSubmit}>
            <div style={{ textAlign:"center", marginBottom:20 }}>
              ...
              <p style={{ color:"#8896aa", fontSize:13 }}>Nhập mã 6 chữ số từ Google Authenticator</p>
            </div>
            ...
            <button type="button" onClick={()=>{setShow2FA(false);setTotpCode("");}} style={{ width:"100%", marginTop:8, padding:"10px", background:"transparent", border:"1px solid rgba(255,255,255,0.08)", borderRadius:10, color:"#8896aa", fontSize:13, cursor:"pointer" }}>← Quay lại</button>
          </form>
        )}
        <p style={{ textAlign:"center", color:"#4a5568", fontSize:13, marginTop:18 }}>
          Chưa có tài khoản?{" "}<Link href="/register" style={{ color:"#4f7cff", textDecoration:"none", fontWeight:600 }}>Đăng ký ngay</Link>
        </p>
      </div>
    </div>
  );
