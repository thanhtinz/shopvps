...
export default function EmailMarketingPage() {
  ...
  const [html, setHtml] = useState(`<h2>${t("Xin chào")} {{name}},</h2>\n<p>${t("Nội dung email của bạn tại đây.")}</p>`);
  ...
  return (
    <div style={{ maxWidth:800 }}>
      <h1 style={{ fontSize:20, fontWeight:800, color:"var(--text-primary)", letterSpacing:"-0.03em", marginBottom:24 }}>{t("Email Marketing")}</h1>
      <div style={{ background:"var(--bg-surface)", border:"1px solid var(--border)", borderRadius:"var(--radius-lg)", padding:"24px" }}>
        <form onSubmit={send}>
          <div style={{ marginBottom:16 }}>
            <label style={{ display:"block", fontSize:11, fontWeight:700, color:"var(--text-muted)", letterSpacing:"0.08em", textTransform:"uppercase", marginBottom:6 }}>{t("Đối tượng")}</label>
            <select value={target} onChange={e=>setTarget(e.target.value)} style={{ ...inputStyle, cursor:"pointer" }}>
              ...
              <option value="USER">{t("Chỉ User")}</option>
              <option value="ADMIN">{t("Chỉ Admin")}</option>
            </select>
          </div>
          <div style={{ marginBottom:16 }}>
            <label style={{ display:"block", fontSize:11, fontWeight:700, color:"var(--text-muted)", letterSpacing:"0.08em", textTransform:"uppercase", marginBottom:6 }}>{t("Tiêu đề email")}</label>
            <input value={subject} onChange={e=>setSubject(e.target.value)} placeholder={t("Thông báo quan trọng từ ShopVPS")} required style={inputStyle} onFocus={e=>e.target.style.borderColor="rgba(79,124,255,0.5)"} onBlur={e=>e.target.style.borderColor="var(--border)"}/>
          </div>
          <div style={{ marginBottom:16 }}>
            <label style={{ display:"block", fontSize:11, fontWeight:700, color:"var(--text-muted)", letterSpacing:"0.08em", textTransform:"uppercase", marginBottom:6 }}>{t("Nội dung HTML (dùng")} &#123;&#123;name&#125;&#125; {t("để cá nhân hoá)")}</label>
          ...
          {result && (
            <div style={{ background:result.success?"var(--green-soft)":"var(--red-soft)", border:`1px solid ${result.success?"rgba(34,197,94,0.2)":"rgba(239,68,68,0.2)"}`, borderRadius:"var(--radius-md)", padding:"10px 14px", marginBottom:16, fontSize:13, color:result.success?"var(--green)":"var(--red)" }}>
              {result.success ? `&#10003; ${t("Đã thêm vào hàng đợi:")} ${result.data?.queued} email` : `&#10007; ${result.error}`}
            </div>
          )}
          <button type="submit" disabled={sending} style={{ padding:"11px 24px", background:sending?"rgba(79,124,255,0.4)":"var(--accent)", border:"none", borderRadius:"var(--radius-md)", color:"white", fontSize:13, fontWeight:700, cursor:sending?"not-allowed":"pointer" }}>
            {sending?t("Đang gửi..."):t("Gửi email →")}
          </button>
        </form>
      </div>
    </div>
