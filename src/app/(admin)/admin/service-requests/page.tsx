...
export default function AdminServiceRequestsPage() {
  ...
  async function act(r: any, action: "approve"|"reject") {
    ...
    if (action === "reject") body.adminNote = prompt("Lý do từ chối (tuỳ chọn):") || undefined;
  ...
  return (
    <div>
      <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:24, flexWrap:"wrap", gap:12 }}>
        <h1 style={{ fontSize:20, fontWeight:800, color:"var(--text-primary)", letterSpacing:"-0.03em" }}>Yêu cầu dịch vụ</h1>
      ...
      <div style={{ background:"var(--bg-surface)", border:"1px solid var(--border)", borderRadius:"var(--radius-lg)", overflow:"hidden" }}>
        {loading ? <div className="skeleton" style={{ height:240, margin:16, borderRadius:"var(--radius-md)" }}/> : items.length===0 ? (
          <div style={{ textAlign:"center", padding:"60px 20px", color:"var(--text-muted)", fontSize:13 }}>Không có yêu cầu nào</div>
        ) : (
          <table style={{ width:"100%", borderCollapse:"collapse" }}>
            <thead><tr style={{ borderBottom:"1px solid var(--border)" }}>
              {["Khách","Loại","Dịch vụ","Chi tiết","Tiền","Trạng thái","Thao tác"].map(h=>(
                ...
                return (
                  <tr key={r.id} style={{ borderBottom:"1px solid var(--border)" }}>
                    <td style={{ padding:"12px 16px" }}>
                      ...
                      <div style={{ fontSize:11.5, color:"var(--text-muted)" }}>Số dư: {formatCurrency(r.user?.balance||0)}</div>
                    </td>
                    <td style={{ padding:"12px 16px" }}><Badge color={typeColor[r.type]||"gray"}>{typeLabel[r.type]||r.type}</Badge></td>
                    <td style={{ padding:"12px 16px" }}>
                    ...
                    <td style={{ padding:"12px 16px", fontSize:12, color:"var(--text-secondary)", maxWidth:220 }}>
                      {r.type==="CANCEL" && <div>{r.cancelMode==="IMMEDIATE"?"Huỷ ngay (hoàn tiền)":"Huỷ cuối kỳ"}</div>}
                    ...
                    <td style={{ padding:"12px 16px", fontSize:13, fontWeight:700, color: amt>0?"var(--red)":amt<0?"var(--green)":"var(--text-muted)" }}>
                      {amt>0?`Thu ${formatCurrency(amt)}`:amt<0?`Hoàn ${formatCurrency(-amt)}`:"—"}
                    </td>
                    ...
                    <td style={{ padding:"12px 16px" }}>
                      {r.status==="PENDING" ? (
                        <div style={{ display:"flex", gap:6, alignItems:"center" }}>
                          {r.type==="ADDON" && (
                            <input value={amts[r.id]??""} onChange={e=>setAmts(p=>({...p,[r.id]:e.target.value}))} placeholder="Phí" style={{ width:70, background:"var(--bg-elevated)", border:"1px solid var(--border)", borderRadius:"var(--radius-sm)", padding:"5px 8px", color:"var(--text-primary)", fontSize:12, outline:"none" }}/>
                          )}
                          <button onClick={()=>act(r,"approve")} disabled={!!busy} style={{ padding:"5px 12px", borderRadius:"var(--radius-sm)", border:"none", background:"var(--green)", color:"white", fontSize:12, fontWeight:600, cursor:"pointer" }}>Duyệt</button>
                          <button onClick={()=>act(r,"reject")} disabled={!!busy} style={{ padding:"5px 12px", borderRadius:"var(--radius-sm)", border:"1px solid var(--border)", background:"var(--bg-elevated)", color:"var(--red)", fontSize:12, fontWeight:600, cursor:"pointer" }}>Từ chối</button>
                        </div>
