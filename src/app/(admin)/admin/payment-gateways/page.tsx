...
export default function AdminGatewaysPage() {
  ...
  const HINTS: Record<string, string> = {
    manual: tr("Không cần cấu hình — dùng tài khoản ngân hàng ở mục Cài đặt. Admin duyệt thủ công."),
    sepay: tr("Không cần cấu hình ở đây — dùng SEPAY_WEBHOOK_SECRET (env) + tài khoản ngân hàng. Cộng tiền tự động."),
  ...
  async function patch(id: string, data: any) {
    ...
    if (!res.ok) { const d = await res.json().catch(()=>({})); alert(d.error || tr("Lỗi")); return; }
    load();
  }
  ...
  return (
    <div>
      <h1 style={{ fontSize:20, fontWeight:800, color:"var(--text-primary)", letterSpacing:"-0.03em", marginBottom:20 }}>{tr("Cổng thanh toán")}</h1>
          ...
          <div key={g.id} style={{ background:"var(--bg-surface)", border:"1px solid var(--border)", borderRadius:"var(--radius-lg)", padding:"18px 20px" }}>
            <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:10 }}>
              <div style={{ display:"flex", alignItems:"center", gap:10 }}>
                ...
                <Badge color={g.isActive?"green":"gray"}>{g.isActive?tr("Đang bật"):tr("Tắt")}</Badge>
            ...
            {(g.code==="stripe"||g.code==="paypal") && (
              <div style={{ display:"flex", gap:8, alignItems:"flex-start" }}>
                ...
                />
                <button onClick={()=>patch(g.id,{ config: drafts[g.id] ?? g.config })} style={{ padding:"9px 14px", background:"var(--accent)", border:"none", borderRadius:"var(--radius-md)", color:"white", fontSize:12.5, fontWeight:600, cursor:"pointer" }}>{tr("Lưu")}</button>
              </div>
            )}
          </div>
        ))}
      ...
      <h2 style={{ fontSize:15, fontWeight:700, color:"var(--text-primary)", marginBottom:12 }}>{tr("Giao dịch nạp chờ duyệt")} ({deposits.length})</h2>
      <div style={{ background:"var(--bg-surface)", border:"1px solid var(--border)", borderRadius:"var(--radius-lg)", overflow:"hidden" }}>
        {deposits.length===0 ? (
          <div style={{ textAlign:"center", padding:"40px 20px", color:"var(--text-muted)", fontSize:13 }}>{tr("Không có giao dịch chờ duyệt")}</div>
        ) : (
          <table style={{ width:"100%", borderCollapse:"collapse" }}>
            <thead><tr style={{ borderBottom:"1px solid var(--border)" }}>
              {[tr("Khách"),tr("Cổng"),tr("Số tiền"),tr("Mã tham chiếu"),tr("Thời gian"),tr("Thao tác")].map(h=>(
                ...
                <tr key={t.id} style={{ borderBottom:"1px solid var(--border)" }}>
                  ...
                  <td style={{ padding:"11px 14px" }}>
                    <div style={{ display:"flex", gap:6 }}>
                      <button onClick={()=>deposit(t.reference,"approve")} style={{ padding:"5px 12px", borderRadius:"var(--radius-sm)", border:"none", background:"var(--green)", color:"white", fontSize:12, fontWeight:600, cursor:"pointer" }}>{tr("Duyệt")}</button>
                      <button onClick={()=>deposit(t.reference,"reject")} style={{ padding:"5px 12px", borderRadius:"var(--radius-sm)", border:"1px solid var(--border)", background:"var(--bg-elevated)", color:"var(--red)", fontSize:12, cursor:"pointer" }}>{tr("Từ chối")}</button>
                    </div>
                  </td>
                </tr>
              ))}
