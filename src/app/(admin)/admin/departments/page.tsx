...
export default function AdminDepartmentsPage() {
  ...
  async function remove(id: string) {
    if (!confirm("Xoá phòng ban?")) return;
    await fetch(`/api/admin/departments/${id}`, { method:"DELETE" }); load();
  }

  return (
    <div style={{ maxWidth:640 }}>
      <h1 style={{ fontSize:20, fontWeight:800, color:"var(--text-primary)", letterSpacing:"-0.03em", marginBottom:20 }}>Phòng ban hỗ trợ</h1>
          ...
          <div key={d.id} style={{ display:"flex", justifyContent:"space-between", alignItems:"center", padding:"13px 18px", borderBottom:"1px solid var(--border)" }}>
            ...
            <div style={{ display:"flex", alignItems:"center", gap:10 }}>
              <button onClick={()=>patch(d.id,{ isActive: !d.isActive })} style={{ border:"none", background:"transparent", cursor:"pointer" }}>
                <Badge color={d.isActive?"green":"gray"}>{d.isActive?"Bật":"Tắt"}</Badge>
              </button>
      ...
      <form onSubmit={add} style={{ display:"flex", gap:10 }}>
        <input value={name} onChange={e=>setName(e.target.value)} placeholder="Tên phòng ban mới..." style={{ flex:1, background:"var(--bg-surface)", border:"1.5px solid var(--border)", borderRadius:"var(--radius-md)", padding:"10px 12px", color:"var(--text-primary)", fontSize:13.5, outline:"none" }}/>
        <button type="submit" style={{ padding:"10px 18px", background:"var(--accent)", border:"none", borderRadius:"var(--radius-md)", color:"white", fontWeight:600, fontSize:13, cursor:"pointer" }}>Thêm</button>
      </form>
    </div>
  );
}
