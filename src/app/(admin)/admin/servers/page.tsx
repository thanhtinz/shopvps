  ...
  return (
    <div>
      <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:24 }}>
        <div>
          <h1 style={{ fontSize:20, fontWeight:800, color:"var(--text-primary)", letterSpacing:"-0.03em", marginBottom:2 }}>{t("WHM Servers")}</h1>
          <p style={{ color:"var(--text-muted)", fontSize:13 }}>{t("Quản lý các server WHM/cPanel")}</p>
        </div>
      ...
      {servers.length === 0 ? (
        <div style={{ textAlign:"center", padding:"80px 20px", background:"var(--bg-surface)", border:"1px solid var(--border)", borderRadius:"var(--radius-xl)" }}>
          ...
          <h3 style={{ fontSize:16, fontWeight:700, color:"var(--text-primary)", marginBottom:8 }}>{t("Chưa có server nào")}</h3>
          <p style={{ color:"var(--text-muted)", fontSize:13, marginBottom:24 }}>{t("Thêm server WHM để bắt đầu bán hosting")}</p>
          <button onClick={()=>setShowForm(true)} style={{ padding:"10px 24px", background:"var(--accent)", border:"none", borderRadius:"var(--radius-md)", color:"white", fontWeight:600, cursor:"pointer" }}>{t("Thêm server ngay")}</button>
        </div>
      ) : (
              ...
              <div style={{ display:"flex", alignItems:"flex-start", justifyContent:"space-between", flexWrap:"wrap", gap:12 }}>
                <div style={{ display:"flex", alignItems:"center", gap:14 }}>
                  ...
                  <div>
                    ...
                    <div style={{ fontSize:12, color:"var(--text-muted)" }}>{t("User:")} {s.whmUser} · {t("Max accounts:")} {s.maxAccounts}</div>
                  </div>
                </div>
                <div style={{ display:"flex", gap:8, alignItems:"center" }}>
                  ...
                  <button onClick={()=>testConnection(s.id)} disabled={testing===s.id} style={{ padding:"7px 14px", background:"var(--bg-elevated)", border:"1px solid var(--border)", borderRadius:"var(--radius-md)", color:"var(--accent)", fontSize:12.5, cursor:"pointer", fontWeight:600 }}>
                    {testing===s.id?t("Đang test..."):t("Test kết nối")}
                  </button>
                  <Badge color={s.isActive?"green":"gray"}>{s.isActive?t("Hoạt động"):t("Tắt")}</Badge>
                </div>
              </div>
            </div>
          ))}
      ...
      {showForm && (
        <div style={{ position:"fixed", inset:0, background:"rgba(0,0,0,0.6)", backdropFilter:"blur(4px)", display:"flex", alignItems:"center", justifyContent:"center", zIndex:50 }} onClick={()=>setShowForm(false)}>
          <div style={{ background:"var(--bg-elevated)", border:"1px solid var(--border)", borderRadius:"var(--radius-xl)", padding:"28px", width:500, maxWidth:"90vw" }} onClick={e=>e.stopPropagation()}>
            <h3 style={{ fontSize:16, fontWeight:700, color:"var(--text-primary)", marginBottom:20 }}>{t("Thêm WHM Server")}</h3>
            <form onSubmit={save}>
              <div style={{ display:"flex", flexDirection:"column", gap:12, marginBottom:16 }}>
                {[
                  { label:t("Tên server"), key:"name", ph:"Server VN #1" },
                  { label:t("WHM Host"), key:"whmHost", ph:"server.example.com" },
                  { label:t("WHM Port"), key:"whmPort", ph:"2087" },
                  { label:t("WHM Username"), key:"whmUser", ph:"root" },
                  { label:t("WHM API Token"), key:"whmToken", ph:"••••••••••••" },
                  { label:t("Max accounts"), key:"maxAccounts", ph:"100" },
                ].map(f => (
                  <div key={f.key}>
              ...
              <div style={{ display:"flex", gap:10 }}>
                ...
                <button type="submit" disabled={saving} style={{ flex:2, padding:"10px", background:"var(--accent)", border:"none", borderRadius:"var(--radius-md)", color:"white", fontWeight:600, cursor:"pointer", fontSize:13 }}>
                  {saving?t("Đang lưu..."):t("Thêm server →")}
                </button>
              </div>
            </form>
          </div>
