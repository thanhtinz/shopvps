  ...
  return (
    <div>
      <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:24, flexWrap:"wrap", gap:12 }}>
        <h1 style={{ fontSize:20, fontWeight:800, color:"var(--text-primary)", letterSpacing:"-0.03em" }}>{t("Tên miền")}</h1>
      ...
      <div style={{ background:"var(--bg-surface)", border:"1px solid var(--border)", borderRadius:"var(--radius-lg)", overflow:"hidden" }}>
        {loading ? <div className="skeleton" style={{ height:200, margin:16, borderRadius:"var(--radius-md)" }}/> : items.length===0 ? (
          <div style={{ textAlign:"center", padding:"50px 20px", color:"var(--text-muted)", fontSize:13 }}>{t("Không có tên miền nào")}</div>
        ) : (
          <table style={{ width:"100%", borderCollapse:"collapse" }}>
            <thead><tr style={{ borderBottom:"1px solid var(--border)" }}>
              {[t("Tên miền"),t("Khách"),t("Loại"),t("Giá"),t("Hết hạn"),t("Trạng thái"),t("Thao tác")].map(h=>(
            ...
            <tbody>
              {items.map(d=>(
                <tr key={d.id} style={{ borderBottom:"1px solid var(--border)" }}>
                  ...
                  <td style={{ padding:"11px 14px", fontSize:12.5, color:"var(--text-secondary)" }}>{d.type==="transfer"?"Transfer":t("Đăng ký")} · {d.years}n</td>
                  ...
                  <td style={{ padding:"11px 14px" }}>
                    <div style={{ display:"flex", gap:6 }}>
                      {d.status!=="ACTIVE" && <button onClick={()=>setStatusFor(d.id,"ACTIVE")} style={{ padding:"5px 11px", borderRadius:"var(--radius-sm)", border:"none", background:"var(--green)", color:"white", fontSize:12, fontWeight:600, cursor:"pointer" }}>{t("Kích hoạt")}</button>}
                      {d.status!=="CANCELLED" && <button onClick={()=>setStatusFor(d.id,"CANCELLED")} style={{ padding:"5px 11px", borderRadius:"var(--radius-sm)", border:"1px solid var(--border)", background:"var(--bg-elevated)", color:"var(--red)", fontSize:12, cursor:"pointer" }}>{t("Huỷ")}</button>}
                    </div>
                  </td>
                </tr>
              ))}
