...
export default function AdminCurrenciesPage() {
  ...
  async function remove(id: string) {
    if (!confirm("Xoá tiền tệ này?")) return;
    const res = await fetch(`/api/admin/currencies/${id}`, { method:"DELETE" });
    if (!res.ok) { const d = await res.json().catch(()=>({})); alert(d.error || "Lỗi"); }
    load();
  }
  ...
  return (
    <div>
      <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:24 }}>
        <div>
          <h1 style={{ fontSize:20, fontWeight:800, color:"var(--text-primary)", letterSpacing:"-0.03em" }}>Tiền tệ</h1>
          <p style={{ fontSize:12.5, color:"var(--text-muted)", marginTop:2 }}>Tỷ giá = số đơn vị tiền cơ sở cho 1 đơn vị tiền này (vd: 1 USD = 24000)</p>
        </div>
        <button onClick={()=>setOpen(true)} style={{ padding:"9px 16px", background:"var(--accent)", border:"none", borderRadius:"var(--radius-md)", color:"white", fontSize:13, fontWeight:600, cursor:"pointer" }}>+ Thêm tiền tệ</button>
      </div>
        ...
        {loading ? <div className="skeleton" style={{ height:200, margin:16, borderRadius:"var(--radius-md)" }}/> : (
          <table style={{ width:"100%", borderCollapse:"collapse" }}>
            <thead><tr style={{ borderBottom:"1px solid var(--border)" }}>
              {["Mã","Tên","Ký hiệu","Tỷ giá","Số lẻ","Trạng thái","Cơ sở","Thao tác"].map(h=>(
              ...
              {items.map(c=>(
                <tr key={c.id} style={{ borderBottom:"1px solid var(--border)" }}>
                  ...
                  <td style={{ padding:"11px 14px" }}>
                    {c.isBase ? <span style={{ fontSize:13, color:"var(--text-muted)" }}>1 (cơ sở)</span> : (
                  ...
                  <td style={{ padding:"11px 14px" }}>
                    <button onClick={()=>patch(c.id,{ isActive: !c.isActive })} style={{ border:"none", background:"transparent", cursor:"pointer" }}>
                      <Badge color={c.isActive?"green":"gray"}>{c.isActive?"Bật":"Tắt"}</Badge>
                    </button>
                  </td>
                  <td style={{ padding:"11px 14px" }}>
                    {c.isBase ? <Badge color="blue">Cơ sở</Badge> : <button onClick={()=>patch(c.id,{ isBase:true })} style={{ fontSize:12, color:"var(--accent)", background:"none", border:"none", cursor:"pointer" }}>Đặt cơ sở</button>}
                  </td>
                  <td style={{ padding:"11px 14px" }}>
                    {!c.isBase && <button onClick={()=>remove(c.id)} style={{ padding:"5px 10px", borderRadius:"var(--radius-sm)", border:"1px solid var(--border)", background:"var(--bg-elevated)", color:"var(--red)", fontSize:12, cursor:"pointer" }}>Xoá</button>}
                  </td>
                </tr>
              ))}
            </tbody>
      ...
      {open && (
        <div style={{ position:"fixed", inset:0, background:"rgba(0,0,0,0.6)", backdropFilter:"blur(4px)", display:"flex", alignItems:"center", justifyContent:"center", zIndex:50 }} onClick={()=>setOpen(false)}>
          <div style={{ background:"var(--bg-elevated)", border:"1px solid var(--border)", borderRadius:"var(--radius-xl)", padding:"26px", width:460, maxWidth:"92vw" }} onClick={e=>e.stopPropagation()}>
            <h3 style={{ fontSize:16, fontWeight:700, color:"var(--text-primary)", marginBottom:18 }}>Thêm tiền tệ</h3>
            <form onSubmit={add} style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:12 }}>
              <Field label="Mã (USD)"><input value={form.code} onChange={e=>setForm({...form,code:e.target.value.toUpperCase()})} required style={inp}/></Field>
              <Field label="Tên"><input value={form.name} onChange={e=>setForm({...form,name:e.target.value})} required style={inp}/></Field>
              <Field label="Ký hiệu ($)"><input value={form.symbol} onChange={e=>setForm({...form,symbol:e.target.value})} style={inp}/></Field>
              <Field label="Tỷ giá (so với cơ sở)"><input type="number" step="any" value={form.rate} onChange={e=>setForm({...form,rate:e.target.value})} disabled={form.isBase} style={inp}/></Field>
              <Field label="Số chữ số thập phân"><input type="number" value={form.decimals} onChange={e=>setForm({...form,decimals:e.target.value})} style={inp}/></Field>
              <Field label="Vị trí ký hiệu">
                <select value={form.position} onChange={e=>setForm({...form,position:e.target.value})} style={inp}>
                  <option value="before">Trước ($100)</option>
                  <option value="after">Sau (100₫)</option>
                </select>
              </Field>
              <label style={{ gridColumn:"1/3", display:"flex", gap:8, fontSize:13, color:"var(--text-secondary)", cursor:"pointer" }}>
                <input type="checkbox" checked={form.isBase} onChange={e=>setForm({...form,isBase:e.target.checked})}/> Đặt làm tiền tệ cơ sở (tỷ giá = 1)
              </label>
              <div style={{ gridColumn:"1/3", display:"flex", gap:10, marginTop:6 }}>
                ...
                <button type="submit" disabled={saving} style={{ flex:2, padding:"10px", background:"var(--accent)", border:"none", borderRadius:"var(--radius-md)", color:"white", fontWeight:600, cursor:"pointer", fontSize:13 }}>{saving?"Đang lưu...":"Thêm"}</button>
              </div>
            </form>
          </div>
        </div>
