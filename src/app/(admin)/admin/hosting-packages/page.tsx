  ...
  return (
    <div>
      <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:24 }}>
        <h1 style={{ fontSize:20, fontWeight:800, color:"var(--text-primary)", letterSpacing:"-0.03em" }}>{t("Gói Hosting")}</h1>
        <button onClick={()=>setShowForm(true)} style={{ display:"flex", alignItems:"center", gap:7, padding:"9px 16px", background:"var(--accent)", border:"none", borderRadius:"var(--radius-md)", color:"white", fontSize:13, fontWeight:600, cursor:"pointer" }}>
          <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round"><path d="M12 5v14M5 12h14"/></svg> {t("Thêm gói")}
        </button>
      </div>
        ...
        <table style={{ width:"100%", borderCollapse:"collapse" }}>
          <thead>
            <tr style={{ borderBottom:"1px solid var(--border)" }}>
              {[t("Tên gói"),t("Server"),t("Tài nguyên"),t("Giá/tháng"),t("Trạng thái"),""].map(h=>(
      ...
      {showForm && (
        <div style={{ position:"fixed", inset:0, background:"rgba(0,0,0,0.6)", backdropFilter:"blur(4px)", display:"flex", alignItems:"center", justifyContent:"center", zIndex:50 }} onClick={()=>setShowForm(false)}>
          <div style={{ background:"var(--bg-elevated)", border:"1px solid var(--border)", borderRadius:"var(--radius-xl)", padding:"28px", width:520, maxWidth:"90vw", maxHeight:"85vh", overflowY:"auto" }} onClick={e=>e.stopPropagation()}>
            <h3 style={{ fontSize:16, fontWeight:700, color:"var(--text-primary)", marginBottom:20 }}>{t("Thêm gói Hosting")}</h3>
            <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:12, marginBottom:16 }}>
              <div style={{ gridColumn:"1/-1" }}>
                <label style={{ display:"block", fontSize:11, fontWeight:700, color:"var(--text-muted)", letterSpacing:"0.08em", textTransform:"uppercase", marginBottom:5 }}>{t("Server WHM")}</label>
                <select value={form.serverId} onChange={e=>setForm(p=>({...p,serverId:e.target.value}))} style={{ ...inputStyle, cursor:"pointer" }}>
                  <option value="">{t("Chọn server...")}</option>
                  {servers.map((s:any)=><option key={s.id} value={s.id}>{s.name} ({s.whmHost})</option>)}
                </select>
              </div>
              {[
                { label:t("Tên gói"), key:"name", type:"text", ph:"Starter" },
                { label:t("Slug"), key:"slug", type:"text", ph:"starter" },
                { label:t("Disk (MB)"), key:"storage", type:"number", ph:"1000" },
                { label:t("Bandwidth (MB)"), key:"bandwidth", type:"number", ph:"10000" },
                { label:t("Database"), key:"databases", type:"number", ph:"10 (0=∞)" },
                { label:t("Email"), key:"emailAccounts", type:"number", ph:"100 (0=∞)" },
                { label:t("Giá/tháng"), key:"priceMonthly", type:"number", ph:"49000" },
                { label:t("Giá/năm"), key:"priceYearly", type:"number", ph:"490000" },
              ].map(f=>(
                <div key={f.key}>
