...
export default function AdminReportsPage() {
  ...
  const cards = [
    { label: "Doanh thu", value: formatCurrency(data.totals.revenue), color: "var(--green)" },
    { label: "Tiền nạp", value: formatCurrency(data.totals.deposits), color: "var(--accent)" },
    { label: "VAT đã thu", value: formatCurrency(data.totals.vatCollected), color: "var(--purple)" },
    { label: "Hoàn tiền", value: formatCurrency(data.totals.refunds), color: "var(--yellow)" },
    { label: "Khách mới", value: data.totals.newUsers, color: "var(--cyan)" },
    { label: "Huỷ dịch vụ (churn)", value: data.totals.churn, color: "var(--red)" },
    { label: "Giá trị đơn TB", value: formatCurrency(data.totals.avgOrderValue), color: "var(--text-primary)" },
  ];
  const products = [
    ...
    { label: "Tên miền", active: data.activeServices.domain, neworders: data.newOrders.domain, color: "var(--green)" },
  ];
  ...
  return (
    <div>
      <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:24, flexWrap:"wrap", gap:12 }}>
        <h1 style={{ fontSize:20, fontWeight:800, color:"var(--text-primary)", letterSpacing:"-0.03em" }}>Báo cáo</h1>
        <div style={{ display:"flex", gap:4, background:"var(--bg-elevated)", borderRadius:"var(--radius-md)", padding:4 }}>
          {[6,12,24].map(m=>(
            <button key={m} onClick={()=>setMonths(m)} style={{ padding:"7px 14px", borderRadius:"var(--radius-sm)", border:"none", cursor:"pointer", fontSize:12.5, fontWeight:600, background:months===m?"var(--bg-surface)":"transparent", color:months===m?"var(--text-primary)":"var(--text-muted)" }}>{m} tháng</button>
          ))}
        </div>
      </div>
      ...
      <div style={{ background:"var(--bg-surface)", border:"1px solid var(--border)", borderRadius:"var(--radius-lg)", padding:"18px 20px", marginBottom:20 }}>
        <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:14 }}>
          <span style={{ fontSize:14, fontWeight:700, color:"var(--text-primary)" }}>Doanh thu theo tháng</span>
          <div style={{ display:"flex", gap:14, fontSize:11.5, color:"var(--text-muted)" }}>
            <span style={{ display:"flex", alignItems:"center", gap:5 }}><i style={{ width:9, height:9, borderRadius:2, background:"var(--green)" }}/>Doanh thu</span>
            <span style={{ display:"flex", alignItems:"center", gap:5 }}><i style={{ width:9, height:9, borderRadius:2, background:"var(--accent)", opacity:0.45 }}/>Tiền nạp</span>
          </div>
        </div>
        <div style={{ position:"relative" }}>
          ...
          {hover !== null && (
            <div style={{ position:"absolute", top:0, left:`${(hover/data.monthly.length)*100}%`, transform:"translateX(-50%)", background:"var(--bg-elevated)", border:"1px solid var(--border)", borderRadius:"var(--radius-md)", padding:"7px 10px", fontSize:11.5, pointerEvents:"none", whiteSpace:"nowrap", zIndex:2 }}>
              ...
              <div style={{ color:"var(--green)" }}>DT: {formatCurrency(data.monthly[hover].revenue)}</div>
              <div style={{ color:"var(--accent)" }}>Nạp: {formatCurrency(data.monthly[hover].deposits)}</div>
              <div style={{ color:"var(--text-muted)" }}>User mới: {data.monthly[hover].newUsers}</div>
            </div>
          )}
        </div>
      ...
      <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:16 }}>
        ...
        <div style={{ background:"var(--bg-surface)", border:"1px solid var(--border)", borderRadius:"var(--radius-lg)", padding:"18px 20px" }}>
          <div style={{ fontSize:14, fontWeight:700, color:"var(--text-primary)", marginBottom:16 }}>Sản phẩm (đơn mới {months} tháng)</div>
          {products.map(p=>(
            <div key={p.label} style={{ marginBottom:14 }}>
              <div style={{ display:"flex", justifyContent:"space-between", fontSize:12.5, marginBottom:5 }}>
                <span style={{ color:"var(--text-primary)", fontWeight:600 }}>{p.label} <span style={{ color:"var(--text-muted)", fontWeight:400, fontSize:11 }}>· {p.active} đang chạy</span></span>
        ...
        {/* Top customers */}
        <div style={{ background:"var(--bg-surface)", border:"1px solid var(--border)", borderRadius:"var(--radius-lg)", padding:"18px 20px" }}>
          <div style={{ fontSize:14, fontWeight:700, color:"var(--text-primary)", marginBottom:14 }}>Khách hàng chi tiêu nhiều</div>
