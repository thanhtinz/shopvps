  ...
  return (
    <div>
      <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:24 }}>
        <div><h1 style={{ fontSize:20, fontWeight:800, color:"var(--text-primary)", letterSpacing:"-0.03em", marginBottom:2 }}>VPS Providers</h1><p style={{ color:"var(--text-muted)", fontSize:13 }}>{t("Quản lý API keys nhà cung cấp VPS")}</p></div>
        ...
        {providers.map(p=>(
          <div key={p.id} style={{ background:"var(--bg-surface)", border:"1px solid var(--border)", borderRadius:"var(--radius-lg)", padding:"18px 20px", display:"flex", alignItems:"center", gap:14 }}>
            ...
            <div style={{ flex:1 }}>
              ...
              <div style={{ fontSize:12, color:"var(--text-muted)", fontFamily:"var(--font-mono)" }}>slug: {p.slug} · API: {p.apiKey || t("Chưa cấu hình")}</div>
            </div>
      ...
      {showForm && (
        <div style={{ position:"fixed", inset:0, background:"rgba(0,0,0,0.6)", backdropFilter:"blur(4px)", display:"flex", alignItems:"center", justifyContent:"center", zIndex:50 }} onClick={()=>setShowForm(false)}>
          <div style={{ background:"var(--bg-elevated)", border:"1px solid var(--border)", borderRadius:"var(--radius-xl)", padding:"28px", width:460, maxWidth:"90vw" }} onClick={e=>e.stopPropagation()}>
            <h3 style={{ fontSize:16, fontWeight:700, color:"var(--text-primary)", marginBottom:20 }}>{t("Thêm VPS Provider")}</h3>
            <form onSubmit={save}>
              <div style={{ marginBottom:12 }}>
                <label style={{ display:"block", fontSize:11, fontWeight:700, color:"var(--text-muted)", letterSpacing:"0.08em", textTransform:"uppercase", marginBottom:5 }}>{t("Provider có sẵn")}</label>
                <select onChange={e=>{ const v=e.target.value; if(v) setForm(p=>({...p,name:slugMap[v]||v,slug:v})); }} style={{ ...inputStyle, cursor:"pointer" }}>
                  <option value="">{t("Chọn hoặc nhập thủ công...")}</option>
                  {Object.entries(slugMap).map(([k,v])=><option key={k} value={k}>{v}</option>)}
                </select>
              </div>
              {[
                { label:t("Tên hiển thị"), key:"name", ph:"Vultr" },
                { label:t("Slug"), key:"slug", ph:"vultr" },
                { label:t("API Key"), key:"apiKey", ph:"••••••" },
                { label:t("API Endpoint (tuỳ chọn)"), key:"apiEndpoint", ph:"https://api.vultr.com/v2" },
              ].map(f=>(
              ...
              <div style={{ display:"flex", gap:10, marginTop:16 }}>
                ...
                <button type="submit" disabled={saving} style={{ flex:2, padding:"10px", background:"var(--accent)", border:"none", borderRadius:"var(--radius-md)", color:"white", fontWeight:600, cursor:"pointer", fontSize:13 }}>{saving?t("Đang lưu..."):t("Thêm provider →")}</button>
              </div>
            </form>
          </div>
        </div>
